...
public class ThirdPersonController : MonoBehaviour
{
    ...
    {
        // Ghost Trial
        if(cc.velocity.magnitude > 0.1f && Time.time - lastUpdateTime > ghostUpdateInterval)
        {
        ...
        {
            isJumping = true;
            if (enableClone && Time.time - lastCloneTime > 1f)
            {
    ...
    private void CloneCharacter()
    {
        ...
        SkinnedMeshRenderer skinnedMeshRenderer = clone.GetComponentInChildren<SkinnedMeshRenderer>();
        skinnedMeshRenderer.materials = ghostMaterials.ToArray();
        ...
        for (int layer = 0; layer < animator.layerCount; layer++)
        {
            ...
            cloneAnimator.Play(stateHash, layer, normalizedTime);
        }
        foreach (AnimatorControllerParameter param in animator.parameters)
        {
            switch (param.type)
            {
                case AnimatorControllerParameterType.Bool:
                    cloneAnimator.SetBool(param.name, animator.GetBool(param.name));
                    break;
            }
        }
    }
